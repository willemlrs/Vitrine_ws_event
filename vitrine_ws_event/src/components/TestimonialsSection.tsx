...
const XTestimonialsSection = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (imageFile) {
      ...
      imageData.append("upload_preset", "ws_event_testimonies");

      try {
        const res = await fetch("https://api.cloudinary.com/v1_1/dub6dwwxo/image/upload", {
          method: "POST",
          body: imageData
        });
    ...
    try {
      const response = await fetch("https://formspree.io/f/xdkgwbbr", {
        method: "POST",
        headers: {
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        {showForm && (
          <div className="bg-muted p-6 rounded-xl mb-10 max-w-xl mx-auto">
            <Textarea
              placeholder={t("testimonialsSection.form.placeholder")}
              value={message}
            ...
            <Input
              type="text"
              placeholder={t("testimonialsSection.form.name_placeholder") || "Votre nom ou pseudo"}
              value={name}
            ...
            <Button onClick={handleSubmit}>
              {t("testimonialsSection.form.submit")}
            </Button>
          </div>
        )}

        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
          {[...userTestimonials, ...initialTestimonials].map((t) => (
            <a
              key={t.id}
              href={t.url}
              target="_blank"
